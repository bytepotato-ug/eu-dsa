...
import { AppealState } from './types.js';
...
export class AppealWorkflow {
    ...
    submit(params) {
        if (!isAppealWindowOpen(params.decisionDate, this.config.window)) {
            throw new Error('Appeal window has expired. Appeals must be filed within 6 months of the decision.');
        }
        return {
     ...
     */
    assign(appeal, reviewerId) {
        this.assertTransition(appeal, AppealState.ASSIGNED);
        if (this.config.enforceDifferentReviewer &&
            appeal.originalDecisionMaker &&
            reviewerId === appeal.originalDecisionMaker) {
            throw new Error('Reviewer cannot be the same person who made the original decision (DSA Art. 20(4)).');
        }
        return {
            ...appeal,
     ...
     */
    resolve(appeal, reviewerId, outcome) {
     ...
     */
    getValidTransitions(appeal) {
        return VALID_TRANSITIONS[appeal.state] ?? [];
    }
    assertTransition(appeal, to) {
        const valid = VALID_TRANSITIONS[appeal.state];
        if (!valid?.includes(to)) {
            throw new Error(`Invalid transition from ${appeal.state} to ${to}. Valid: ${valid?.join(', ') || 'none'}`);
        }
    }
}
