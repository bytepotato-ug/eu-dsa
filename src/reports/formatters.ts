 ...
 */
export function toCSV(report: TransparencyReport): CSVParts {
  const id = report.identification;

  // Part 1: Report Identification
  const part1 = toCSVRows(
    ['Field', 'Value'],
    [
  ...
  const part3 = toCSVRows(
    ['Member State', 'Category', 'Orders Received', 'Orders Complied', 'Median Response (hours)'],
    report.authorityOrders.entries.map(e => [
  ...
  const part4 = report.notices
    ? toCSVRows(
        ['Member State', 'Category', 'Source Type', 'Total Received', 'Action Taken', 'No Action', 'Basis: Illegal', 'Basis: TOS', 'Median Handling (hours)', 'Automated Processing'],
        report.notices.entries.map(e => [
  ...
  const part5 = toCSVRows(
    ['Category', 'Member State', 'Items Actioned', 'Restriction Type', 'Automated Detection', 'Automated Decision'],
    report.ownInitiativeIllegal.entries.map(e => [
  ...
  const part6 = report.ownInitiativeTos
    ? toCSVRows(
        ['Category', 'Member State', 'Items Actioned', 'Restriction Type', 'Automated Detection', 'Automated Decision', 'Trusted Flagger Notices'],
        report.ownInitiativeTos.entries.map(e => [
  ...
  const part8 = toCSVRows(
    ['Tool Name', 'Purpose', 'Description', 'Categories', 'Safeguards'],
    report.automation.tools.map(t => [
  ...
  const part9 = report.humanResources
    ? toCSVRows(
        ['Language', 'Moderator Count', 'Qualifications'],
        report.humanResources.entries.map(e => [
  ...
  // Part 10: AMAR
  const part10 = toCSVRows(
    ['Tool Name', 'Language', 'Accuracy Indicator', 'Error Rate', 'False Positive Rate', 'False Negative Rate'],
    report.amar.entries.map(e => [
      e.toolName, e.language, e.accuracyIndicator, e.errorRate, e.falsePositiveRate ?? '', e.falseNegativeRate ?? '',
    ]),
  );

  // Part 11: Qualitative
  const part11 = toCSVRows(
    ['Section', 'Content'],
    [
 ...
 */
export function toJSON(report: TransparencyReport): string {
 ...
 */
export function toMarkdown(report: TransparencyReport): string {
  const id = report.identification;
  const lines: string[] = [];
  ...
  lines.push('---');
  lines.push(`*Generated by dsa-toolkit on ${report.generatedAt.toISOString().split('T')[0]}*`);
