...
/** Simplified IPv6 pattern (common forms) */
const IPV6_REGEX = /\b(?:[0-9a-fA-F]{1,4}:){2,7}[0-9a-fA-F]{1,4}\b/g;

/**
 * Strip IP addresses from text.
 */
export function stripIpAddresses(text: string): string {
  return text
 ...
 */
export function stripUserUrls(text: string, patterns?: RegExp[]): string {
  let result = text;
  if (patterns) {
 ...
 */
export function sanitizeForSubmission(
  text: string,
  options?: {
    replacements?: Map<string, string>;
    stripIps?: boolean;
  ...
  let result = text;
  if (options?.replacements) result = pseudonymizeText(result, options.replacements);
  if (options?.stripIps !== false) result = stripIpAddresses(result);
  if (options?.stripEmailAddresses !== false) result = stripEmails(result);
  if (options?.stripUserMentions) result = stripMentions(result);
  if (options?.urlPatterns) result = stripUserUrls(result, options.urlPatterns);
  return result;
}
